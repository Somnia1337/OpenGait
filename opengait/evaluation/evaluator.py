...
def cross_view_gallery_evaluation(
    ...
    """
    probe_seq_dict = {
        "CASIA-B": {"NM": ["nm-01"], "BG": ["bg-01"], "CL": ["cl-01"]},
        "OUMVLP": {"NM": ["00"]},
    }

    gallery_seq_dict = {"CASIA-B": ["nm-02", "bg-02", "cl-02"], "OUMVLP": ["01"]}
    ...
    for type_, probe_seq in probe_seq_dict[dataset].items():
        acc[type_] = np.zeros(len(view_list)) - 1.0
        mean_ap[type_] = np.zeros(len(view_list)) - 1.0
...
def single_view_gallery_evaluation(
    ...
    for type_, probe_seq in probe_seq_dict[dataset].items():
        acc[type_] = np.zeros((view_num, view_num, num_rank)) - 1.0
        for v1, probe_view in enumerate(view_list):
            pseq_mask = np.isin(seq_type, probe_seq) & np.isin(view, probe_view)
            pseq_mask = (
                pseq_mask
                if "SUSTech1K" not in dataset
                else np.any(
                    np.asarray(
    ...
    for rank in range(num_rank):
        ...
        for type_ in probe_seq_dict[dataset].keys():
            ...
            if rank == 0:
                ...
                result_dict[f"scalar/test_accuracy/{type_}@R{rank + 1}"] = np.mean(
                    sub_acc
                )
...
def evaluate_Gait3D(data, dataset, metric="euc"):
    ...
    features, labels, cams, time_seqs = (
        data["embeddings"],
        data["labels"],
        data["types"],
        data["views"],
    ...
    probe_sets = json.load(open("./datasets/Gait3D/Gait3D.json", "rb"))["PROBE_SET"]
    probe_mask = []
...
def evaluate_CCPG(data, dataset, metric="euc"):
    ...
    result_dict = {}
    for i in range(1):
        msg_mgr.log_info("===Rank-%d (Include identical-view cases)===" % (i + 1))
        msg_mgr.log_info(
            "CL: %.3f,\tUP: %.3f,\tDN: %.3f,\tBG: %.3f"
            % (
                np.mean(acc[0, :, :, i]),
    ...
    for i in range(1):
        msg_mgr.log_info("===Rank-%d (Exclude identical-view cases)===" % (i + 1))
        msg_mgr.log_info(
            "CL: %.3f,\tUP: %.3f,\tDN: %.3f,\tBG: %.3f"
            % (
                de_diag(acc[0, :, :, i]),
...
def evaluate_FreeGait(data, dataset, metric="euc"):
    ...
    probe_sets = json.load(open("./datasets/FreeGait/FreeGait.json", "rb"))["PROBE_SET"]
