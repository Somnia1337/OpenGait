...
def compute_ACC_mAP(distmat, q_pids, g_pids, q_views=None, g_views=None, rank=1):
    ...
    for q_idx in range(num_q):
        ...
        assert len(q_idx_glabels) > 0, (
            "No gallery after excluding identical-view cases!"
        )
...
def evaluate_rank(distmat, p_lbls, g_lbls, max_rank=50):
    ...
    all_AP = []
    all_INP = []
    num_valid_p = 0.0  # number of valid probe
    ...
    for p_idx in range(num_p):
        ...
        pos_idx = np.where(
            raw_cmc == 1
        )  # 返回坐标，此处raw_cmc为一维矩阵，所以返回相当于index
