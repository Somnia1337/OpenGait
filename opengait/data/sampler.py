...
class TripletSampler(tordata.Sampler):
    def __init__(self, dataset, batch_size, batch_shuffle=False):
        ...
        if (self.batch_size[0] * self.batch_size[1]) % self.world_size != 0:
            raise ValueError(
                "World size ({}) is not divisible by batch_size ({} x {})".format(
    ...
    def __iter__(self):
        while True:
            ...
            total_size = (
                int(math.ceil(total_batch_size / self.world_size)) * self.world_size
            )
...
class InferenceSampler(tordata.Sampler):
    def __init__(self, dataset, batch_size):
        ...
        if batch_size != 1:
            complement_size = math.ceil(self.size / batch_size) * batch_size
...
class CommonSampler(tordata.Sampler):
    def __init__(self, dataset, batch_size, batch_shuffle):
        self.dataset = dataset
        self.size = len(dataset)
        self.batch_size = batch_size
        if not isinstance(self.batch_size, int):
            raise ValueError("batch_size shoude be (B) not {}".format(batch_size))
        self.batch_shuffle = batch_shuffle

        self.world_size = dist.get_world_size()
        if self.batch_size % self.world_size != 0:
            raise ValueError(
                "World size ({}) is not divisble by batch_size ({})".format(
...
class BilateralSampler(tordata.Sampler):
    def __init__(self, dataset, batch_size, batch_shuffle=False):
        self.dataset = dataset
        self.batch_size = batch_size
        self.batch_shuffle = batch_shuffle
    ...
    def __iter__(self):
        ...
        while True:
            ...
            sampled_indices = self.total_indices[
